...
import { clearAllUserErrors, loginUser } from "../store/slice/userSlice";
...
type LoginFormProps = z.infer<typeof loginUserSchema>;
...
export default function Login() {
  ...
  const loginHandleSubmitForm = (data: LoginFormProps) => {
    dispatch(
      loginUser({
        email: data.email,
  ...
  return (
    <div className="flex h-[100vh] w-full justify-center bg-dark">
      ...
      <div className="relative flex w-[700px] flex-col rounded-l-[40px] rounded-r-[40px] bg-white p-20 lg:rounded-l-[40px] lg:rounded-r-none">
        ...
        <form
          onSubmit={handleSubmit(loginHandleSubmitForm)}
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1 sm:flex sm:flex-row">
            <p className="font-opensans text-[#636363]">Não tem uma conta? </p>
            <Link
              to="/create-login"
